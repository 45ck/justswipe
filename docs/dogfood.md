...
- Failure recovery UI smoke passed: `ui:smoke:failure` creates an isolated handoff, submits it, marks the bridge event failed, verifies the visible failure banner/error, retries from the thread log, confirms the bridge event returns to `queued`, then marks the retry sent and verifies the UI returns to `Codex resumed`.
...
- Natural notes-app dogfood passed locally: vague idea was sent through JustSwipe to `E:\justswipe-greenfield-notes-lab`, Codex emitted planning handoff `handoff-mqypztmx-o1fble`, swipe response `Build capture, list, and local save first` relayed back, Codex built `index.html`, `styles.css`, and `app.js`, independent browser smoke verified add/list/complete/delete/localStorage/mobile overflow/no console errors, target repo committed at `32d8f92`, and local dogfood was restored to `E:\justswipe` with queued/running/failed bridge events all `0`.
- Self-dogfood trust cue improvement passed locally: JustSwipe idea `idea-mqyquxo0-ime6zw` routed to thread `019f11af-f4b9-76b2-ac6e-0ecbb4ab2dad`, Codex added the sent-state resume evidence strip, `ui:smoke` now asserts `Swipe saved`, `Bridge relay`, and `Codex continues`, and build/UI smoke/bridge smoke/local doctor all passed before returning queued/running/failed bridge events to `0`.
- Multi-shape card bundle smoke passed locally: `ui:smoke:card-shapes` used handoff `handoff-mqyrl43j-shjikz` to verify yes/no, free text, adaptive form, unsupported schema fallback, options-style More action, inline HTML preview, and four-card bundle ordering through mobile-width Chromium before final bridge status returned queued/running/failed events to `0`.
- Natural habit-app dogfood passed locally: vague idea was sent through JustSwipe to `E:\justswipe-greenfield-habit-lab`, Codex emitted planning handoff `handoff-mqys8mhd-c3jcmv`, swipe response `Build today focus` relayed back, Codex built a static Habit Lab app, emitted review handoff `handoff-mqysjbch-kuknpb`, review response `Keep first slice` relayed back, independent Playwright smoke verified commit/done/localStorage/reset/mobile/no console errors, target repo committed at `8c2f255`, and local dogfood restored to `E:\justswipe` with queued/running/failed bridge events all `0`.
- Active-relay stale-heartbeat smoke passed locally: `ui:smoke:relay-state` used handoff `handoff-mqytmuk4-fq5cld` to verify a running relay is shown as `Codex resuming`, explains stale heartbeat during active work is not by itself a failure, does not show `Bridge not observed`, and does not show `Start watcher`; final local status returned queued/running/failed events to `0`.
- Multi-thread project view smoke passed locally: `ui:smoke:multi-thread` verifies multiple tracked thread rows, queued existing-thread idea, queued new-thread idea, idle thread row, search, active/empty waiting filters, project filter, and selecting an existing thread as the idea target.
